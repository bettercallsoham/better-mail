...
const LABEL_META: Record<string, {
  ...
  bg: string; text: string;           // light mode
  darkBg: string; darkText: string;   // dark mode
}> = {
  CATEGORY_PERSONAL:   { icon: "👤", name: "Personal",   bg: "rgba(99,102,241,0.08)",  text: "#4f46e5", darkBg: "rgba(99,102,241,0.15)",  darkText: "#818cf8" },
  CATEGORY_PROMOTIONS: { icon: "🏷",  name: "Promotions", bg: "rgba(120,113,108,0.09)", text: "#78716c", darkBg: "rgba(255,255,255,0.07)", darkText: "rgba(255,255,255,0.42)" },
  CATEGORY_UPDATES:    { icon: "🔔", name: "Updates",    bg: "rgba(100,116,139,0.08)", text: "#475569", darkBg: "rgba(100,116,139,0.15)", darkText: "#94a3b8" },
  CATEGORY_SOCIAL:     { icon: "💬", name: "Social",     bg: "rgba(16,185,129,0.08)",  text: "#059669", darkBg: "rgba(16,185,129,0.14)",  darkText: "#34d399" },
  CATEGORY_FORUMS:     { icon: "📋", name: "Forums",     bg: "rgba(139,92,246,0.08)",  text: "#7c3aed", darkBg: "rgba(139,92,246,0.14)",  darkText: "#a78bfa" },
  IMPORTANT:           { icon: "⚡", name: "Important",  bg: "rgba(217,119,6,0.08)",   text: "#b45309", darkBg: "rgba(217,119,6,0.15)",   darkText: "#fbbf24" },
};
...
function ActionTray({
  thread, onStar, onMarkRead, onArchive, size = "default",
}: {
  thread: ThreadEmail;
      ...
      <div className="flex items-center gap-px shrink-0">
        <Tooltip>
          <TooltipTrigger asChild>
            <button onClick={onStar} className={cn(btnCls, thread.isStarred && "text-amber-400 hover:text-amber-500")} aria-label={thread.isStarred ? "Unstar" : "Star"}>
    ...
    <div
      ...
      className={cn(
        ...
        isActive && [
          ...
          "before:bg-zinc-900 dark:before:bg-white/55 before:rounded-r-full",
        ],
      )}
    >
      <div className="w-2 shrink-0 flex justify-center">
        {isUnread && <span className="block w-1.5 h-1.5 rounded-full bg-zinc-900 dark:bg-white/65" />}
      </div>
      ...
      <div className="relative flex items-center justify-end shrink-0 w-[120px]">
        <span className={cn(
          ...
          isUnread ? "text-gray-500 dark:text-white/42" : "text-gray-400 dark:text-white/22",
          (isFocused || undefined) && "opacity-0",
          !isFocused && "group-hover:opacity-0",
        )}>
          {thread.isStarred && <Star size={10} className="inline-block fill-amber-400 text-amber-400 mr-1 mb-px" />}
          {formatThreadDate(thread.receivedAt)}
        </span>
        <div className={cn("absolute right-0 transition-opacity duration-100", isFocused ? "opacity-100" : "opacity-0 group-hover:opacity-100")}>
          <ActionTray thread={thread} onStar={stop(star)} onMarkRead={stop(markRead)} onArchive={stop(archiveToggle)} size="sm" />
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isActive && "bg-black/[0.05] dark:bg-white/[0.07] dark:ring-1 dark:ring-white/[0.09]",
      )}
    >
      <span
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-[3px]">
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex items-center justify-end shrink-0 h-5">
            <div className={cn("flex items-center gap-1.5 transition-opacity duration-100", isFocused ? "opacity-0" : "group-hover:opacity-0")}>
              {labels.length > 0 && (
                <div className="flex items-center gap-0.5">
                  {labels.slice(0, 3).map((l) => <LabelDot key={l} label={l} />)}
                </div>
              )}
              {thread.isStarred && <Star size={10} className="fill-amber-400 text-amber-400 shrink-0" />}
              <span className={cn(
