...
function CollapsedRow({
  ...
  return (
    ...
    >
      ...
      <span className="text-[11px] text-gray-400 dark:text-white/22 tabular-nums shrink-0">
        {date}
      </span>
      <IconChevronDown
        size={13}
...
function OpenCard({
  ...
  const hasAttachments = !!(email as FullEmail & { attachments?: unknown[] })
    .attachments?.length;
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          >
            {email.isStarred ? (
              ...
              <IconStar
                size={14}
                className="text-gray-300 dark:text-white/18 hover:text-amber-400 transition-colors"
              />
            )}
          </button>
          <span className="text-[11px] tabular-nums text-gray-400 dark:text-white/25">
            {date}
          </span>
          <button
            onClick={onCollapse}
...
export function EmailCard({ email, defaultOpen, onStar }: EmailCardProps) {
  ...
  return (
    <OpenCard email={email} onCollapse={() => setOpen(false)} onStar={onStar} />
  );
}
