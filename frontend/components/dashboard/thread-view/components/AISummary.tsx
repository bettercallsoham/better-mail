...
import {
  IconSparkles, IconCopy, IconCheck,
...
export function AISummary({ threadId, emailAddress, variant = "card" }: AISummaryProps) {
  ...
  if (variant === "bar") {
    ...
    return (
      <div className="shrink-0 flex items-start gap-2.5 px-5 py-2.5 bg-violet-50/60 dark:bg-violet-950/15 border-b border-violet-100/60 dark:border-violet-900/25">
        ...
        <p className="flex-1 text-[12px] text-gray-600 dark:text-white/50 leading-relaxed">{s!.text}</p>
        {s?.actionItems && (
      ...
      >
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          <span className="w-5 h-5 flex items-center justify-center">
            <IconChevronDown
              size={13}
              className={cn("text-gray-300 dark:text-white/20 transition-transform duration-200", expanded && "rotate-180")}
            />
          </span>
        </div>
      </button>
