...
// ─────────────────────────────────────────────────────────────────────────────
// Keyboard shortcut aliases — single key → operator expansion
// Exported so FilterChips can show shortcut badges in the Status dropdown
// ─────────────────────────────────────────────────────────────────────────────
...
const PROMOTABLE: Array<{ match: string; apply: (f: ActiveFilters) => ActiveFilters }> = [
...
function detectInProgressOperator(raw: string): { prefix: "from" | "to"; partial: string } | null {
  if (!raw.includes(":")) return null;
  const match = raw.match(/(?:^|\s)(from:|to:)(\S*)$/i);
  if (!match) return null;
  return { prefix: match[1].replace(":", "").toLowerCase() as "from" | "to", partial: match[2] };
}
...
function buildSearchParams(
  ...
  const merged: SearchQueryParams = { ...filters, size, query: parsed?.query?.trim() || " " };
...
function dedupeRecentSearches<T extends { searchText: string }>(items: T[]): T[] {
  ...
  return meaningful.filter(
    (r) => !meaningful.some(
      (other) => other.searchText !== r.searchText &&
...
export function useSearchState(open: boolean, onOpenChange: (v: boolean) => void) {
  ...
  const filterCount  = useMemo(() => Object.values(filters).filter((v) => v !== undefined && v !== "").length, [filters]);
  ...
  const searchParams = useMemo(() => (inProgress ? null : buildSearchParams(parsed, filters, 8)), [inProgress, parsed, filters]);

  // Ghost text suffix — shown when input is exactly a shortcut letter
  ...
  const recentSearches    = useMemo(() => dedupeRecentSearches(recentData?.data ?? []).slice(0, 6), [recentData]);
  ...
  // Commit a saved search — applies its stored query + filters
  const commitSavedSearch = useCallback((saved: SavedSearch) => {
  ...
  const commitInlineSuggestion = useCallback((email: string) => {
    ...
    const cleaned = input
      ...
      .replace(/\s{2,}/g, " ").trim();
    dispatchFilter({ type: "PATCH", patch: prefix === "from" ? { filterFrom: email } : { filterTo: email } });
    setInputRaw(cleaned);
  }, [inProgress, input]);
