...
export function stripMark(html: string) {
  return html.replace(/<\/?mark[^>]*>/g, "");
}

export function applyHighlight(html: string) {
  return html.replace(
    /<mark>(.*?)<\/mark>/g,
...
function SaveBtn({ text }: { text: string }) {
...
export function RecentRow({ text, onClick }: { text: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
...
export function ThreadRow({
  from, subject, snippet, date, isUnread, hasAttachment, isStarred,
  focused, onHover, onClick,
}: {
  from: string; subject: string; snippet?: string; date: string;
  isUnread: boolean; hasAttachment?: boolean; isStarred?: boolean;
  focused: boolean; onHover: () => void; onClick: () => void;
}) {
  return (
    <button
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-1.5 min-w-0">
        <span
          ...
          dangerouslySetInnerHTML={{ __html: applyHighlight(subject) }}
        />
        {snippet && (
          <>
      ...
      <div className="flex items-center gap-1.5 shrink-0 text-gray-300 dark:text-white/20 ml-auto pl-2">
        ...
        <span className="text-[11.5px] text-gray-400 dark:text-white/28 tabular-nums">{fmtDate(date)}</span>
      </div>
    </button>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-2 py-14 text-center px-6">
      <p className="text-[13px] text-gray-400 dark:text-white/30">
        No results for &ldquo;<span className="text-gray-600 dark:text-white/50">{query}</span>&rdquo;
      </p>
